...
System.out.println(String.format("%s\t\t%s\t\t%s\t%s\t%s","Time_t","P_size","accepted","sent","rem
 aining")); 
for (int i = 0; i < n; i++) { 
if (buf[i] != 0) { 
...
bucket_remaining += buf[i]; 
            ...
            if (received == -1) { 
                System.out.println(String.format("%d\t\t%d\t\t%s\t\t%d\t\t%d", i + 1, buf[i], "dropped", 
sent, bucket_remaining)); 
            } else { 
                System.out.println(String.format("%d\t\t%d\t\t%d\t\t%d\t\t%d", i + 1, buf[i], received, sent, 
bucket_remaining)); 
            } 
        } 
    } 
